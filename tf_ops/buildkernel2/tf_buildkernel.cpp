...
REGISTER_OP("SphericalKernel")
    ...
    .Output("rotate_xyz: float32")   // kernel bin indices: batch * mpoint * nn_sample
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
        ...
        //B*M*K*3
        ::tensorflow::shape_inference::ShapeHandle rotate_xyz = c->MakeShape({c->Dim(nn_index, 0), c->Dim(nn_index, 1), c->Dim(nn_index, 2),3});
        c->set_output(1, rotate_xyz);
        return Status::OK();
    });
...
class SphericalKernelGpuOp : public OpKernel {
    public:
        ...
        void Compute(OpKernelContext* context) override {
            ...
            Tensor* rotate_xyz_tensor = NULL;
            OP_REQUIRES_OK(context, context->allocate_output(1, TensorShape{B,M,K,3}, &rotate_xyz_tensor));
            auto rotate_xyz_flat = rotate_xyz_tensor->flat<float>();
            ...
            float* rotateXyz = &(rotate_xyz_flat(0));
            cudaMemset(filtIndex, 0, sizeof(int)*B*M*K);
            cudaMemset(rotateXyz, 0, sizeof(int)*B*M*K*3);
