...
__device__ void calcRotateMat(float *RotateMat,const float *dircRef, const float *input)
{
	//RotateMat: 3*3 the SVD resilt for rotation
	//dircRef：3*1 the direction for sign flip
	//input: the point (with mean = 0) to rotation
	...
	//sign filp of u 
	//make sure dircRef*u[x & y]> 0 if possible (in orthogonal constraint)
	{
		float meanU[3]={0,0,0};
		for(int i=0;i<3;i++)
			for(int j=0;j<3;j++)
			meanU[i] += dircRef[j]*trans[i+3*j];
