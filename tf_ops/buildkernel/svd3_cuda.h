...
__device__ void calcRotateMat(float *RotateMat,const float *xyzmean,const float *refmean, const float *input)
{
	float *trans = RotateMat;
	const float *mean = xyzmean;
	//x = x - mean(x)
	// for(int i=0;i<3;i++)
	// 	mean[i] = 0;
    ...
    //         for(int k = 0;k < numpoint;k++)
    //         {
    //             input[i*3+j] += (xyz[3*k+i]-mean[i])*(xyz[3*k+j]-mean[j]);//xyz[3*k:3*k+2] a point
    //         }
    //         input[i+j*3] = input[i*3+j];
    //     }
    // }
	// for(int i =0;i<9;i++)
	// 	input[i] = input[i]/numpoint;

	//[u,s,v] = svd(input) u = v and are orthogonal mats
	svd(
        input[0], input[1],input[2],input[3],input[4],input[5],input[6],input[7],input[8],
        trans[0], trans[1],trans[2],trans[3],trans[4],trans[5],trans[6],trans[7],trans[8],//u
        trans[9], trans[10],trans[11],//s
        trans[12],trans[13],trans[14],trans[15],trans[16],trans[17],trans[18],trans[19],trans[20]//v
		);

	//sign filp of u 
	//make sure mean*u[x & y]> 0 if possible (in orthogonal constraint)
	{
		float meanU[3]={0,0,0};
		for(int i=0;i<3;i++)
			for(int j=0;j<3;j++)
			meanU[i] += (mean[j] - refmean[j])*trans[i+3*j];
