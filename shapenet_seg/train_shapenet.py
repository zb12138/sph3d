...
factor = np.int32(640/class_size)+1
new_class_size = factor*class_size
...
print('class %s: (class ID #%d, part number #%d, new_cls_size #%d)'%(shape_name,cls,NUM_CLASSES,
                                                                     new_class_size[cls]))
if factor[cls]>1:
    for j in range(len(trainlist)):
        for _ in range(factor[cls]-1):
            trainlist.append(trainlist[j])
    for j in range(len(testlist)):
        for _ in range(factor[cls]-1):
            testlist.append(testlist[j])
print('factor:%d'%factor[cls])
print('trainlist_repmat:',len(trainlist))
print('testlist_repmat:',len(testlist))

DECAY_STEP = new_class_size[cls]*36
    ...
    while True:
        try:
            ...
            for b in range(bsize):
                ...
                #print(padded_all[b,num-2:num,-1])
